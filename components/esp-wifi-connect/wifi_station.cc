    ...
    for (int i = 0; i < ap_num; i++) {
        ...
        if (it != ssid_list.end()) {
            ...
            WifiApRecord record = {
                ...
                .bssid = {0},
                .eap_identity = it->eap_identity,
                .eap_username = it->eap_username,
            };
...
void WifiStation::StartConnect() {
    ...
    // For enterprise networks the PSK field is unused; leave it empty.
    if (ap_record.eap_username.empty()) {
        strcpy((char *)wifi_config.sta.password, ap_record.password.c_str());
    } else {
        // KamiMon: enterprise APs (e.g. eduroam) advertise PMF and a higher
        // authmode. Zero-init defaults filter them as "incompatible security"
        // (reason 210). Tell the supplicant we accept WPA2-Enterprise+ and
        // are PMF-capable so the AP isn't rejected before EAP starts.
    ...
    // KamiMon: configure WPA2/3-Enterprise (PEAP-MSCHAPv2) when the saved
    // entry carries an EAP username; otherwise disable any prior enterprise
    // state so a subsequent personal SSID connects cleanly.
    if (!ap_record.eap_username.empty()) {
        ESP_LOGI(TAG, "Connecting to %s via WPA2-Enterprise (PEAP, user=%s)",
                 ap_record.ssid.c_str(), ap_record.eap_username.c_str());
        const auto& ident = ap_record.eap_identity;
        const auto& user = ap_record.eap_username;
        const auto& pass = ap_record.password;
        if (!ident.empty()) {
            ESP_ERROR_CHECK(esp_eap_client_set_identity(
                reinterpret_cast<const uint8_t*>(ident.c_str()), ident.size()));
        } else {
            esp_eap_client_clear_identity();
        }
        ESP_ERROR_CHECK(esp_eap_client_set_username(
            reinterpret_cast<const uint8_t*>(user.c_str()), user.size()));
        ESP_ERROR_CHECK(esp_eap_client_set_password(
            reinterpret_cast<const uint8_t*>(pass.c_str()), pass.size()));
        // No CA cert pinned — eduroam servers vary by realm; trade strict
        // server validation for usability. TODO(KamiMon): allow uploading
        // a CA cert via the captive portal for users who want validation.
        esp_eap_client_clear_ca_cert();
        // Be explicit about PEAP — the most common eduroam EAP method.
        ESP_ERROR_CHECK(esp_eap_client_set_eap_methods(ESP_EAP_TYPE_PEAP));
        ESP_ERROR_CHECK(esp_wifi_sta_enterprise_enable());
    } else {
        // Clear any leftover enterprise state from a previous connection.
