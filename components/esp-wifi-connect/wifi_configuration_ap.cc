...
#include <nvs_flash.h>
#include <cJSON.h>
#include <esp_eap_client.h>   // KamiMon: WPA2/3-Enterprise (PEAP-MSCHAPv2)
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // KamiMon: optional WPA2-Enterprise (PEAP) fields.
            cJSON *eap_identity_item = cJSON_GetObjectItemCaseSensitive(json, "eap_identity");
            cJSON *eap_username_item = cJSON_GetObjectItemCaseSensitive(json, "eap_username");
            ...
            if (cJSON_IsString(password_item) && (password_item->valuestring != NULL) && (strlen(password_item->valuestring) < 129)) {
                // EAP passwords can exceed the 64-char WPA-PSK limit; allow up to 128.
                password_str = password_item->valuestring;
            }
            std::string eap_identity_str = "";
            if (cJSON_IsString(eap_identity_item) && eap_identity_item->valuestring != NULL && strlen(eap_identity_item->valuestring) < 65) {
                eap_identity_str = eap_identity_item->valuestring;
            }
            std::string eap_username_str = "";
            if (cJSON_IsString(eap_username_item) && eap_username_item->valuestring != NULL && strlen(eap_username_item->valuestring) < 65) {
                eap_username_str = eap_username_item->valuestring;
            }

            // 获取当前对象
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            if (!this_->ConnectToWifi(ssid_str, password_str, eap_identity_str, eap_username_str)) {
                cJSON_Delete(json);
                httpd_resp_send(req, "{\"success\":false,\"error\":\"Failed to connect to the Access Point\"}", HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }

            this_->Save(ssid_str, password_str, eap_identity_str, eap_username_str);
            cJSON_Delete(json);
            // 设置成功响应
            httpd_resp_set_type(req, "application/json");
            httpd_resp_set_hdr(req, "Connection", "close");
                                         ...
                                         const std::string &eap_username)
{
    ...
    // For WPA2-Personal the IDF wifi_config field caps at 64 chars; for
    // enterprise we route the password through esp_eap_client_set_password
    // and only need it to fit in the EAP buffer (typically <=128).
    const bool enterprise = !eap_username.empty();
    if (!enterprise && password.length() > 64) {
        ESP_LOGE(TAG, "Password too long");
        return false;
    }
    if (enterprise && password.length() > 128) {
        ESP_LOGE(TAG, "EAP password too long");
        return false;
    }
    ...
    if (!enterprise) {
        ...
        // KamiMon: see wifi_station.cc — without these the supplicant
        // filters enterprise APs out before EAP starts (reason 210).
    ...
    // KamiMon: configure WPA2/3-Enterprise (PEAP-MSCHAPv2) when the user
    // supplied an EAP username. This must happen before esp_wifi_connect so
    // the supplicant picks up the credentials on association.
    if (enterprise) {
        ESP_LOGI(TAG, "Connecting to %s via WPA2-Enterprise (PEAP, user=%s)",
                 ssid.c_str(), eap_username.c_str());
        if (!eap_identity.empty()) {
            ESP_ERROR_CHECK(esp_eap_client_set_identity(
                reinterpret_cast<const uint8_t*>(eap_identity.c_str()), eap_identity.size()));
        } else {
            esp_eap_client_clear_identity();
        }
        ESP_ERROR_CHECK(esp_eap_client_set_username(
            reinterpret_cast<const uint8_t*>(eap_username.c_str()), eap_username.size()));
        ESP_ERROR_CHECK(esp_eap_client_set_password(
            reinterpret_cast<const uint8_t*>(password.c_str()), password.size()));
        // No CA cert pinned — see comment in wifi_station.cc StartConnect.
        esp_eap_client_clear_ca_cert();
        ESP_ERROR_CHECK(esp_eap_client_set_eap_methods(ESP_EAP_TYPE_PEAP));
        ESP_ERROR_CHECK(esp_wifi_sta_enterprise_enable());
    } else {
        // Personal SSID: clear any prior enterprise state.
...
void WifiConfigurationAp::Save(const std::string &ssid, const std::string &password,
                                ...
                                const std::string &eap_username)
{
    ESP_LOGI(TAG, "Save SSID %s %d (enterprise=%d)",
             ssid.c_str(), (int)ssid.length(), eap_username.empty() ? 0 : 1);
    if (!eap_username.empty()) {
        SsidManager::GetInstance().AddEnterpriseSsid(ssid, eap_identity, eap_username, password);
    } else {
